...
import { executeCommand, validateInputs } from '../src/utilities'
...
describe('utilities tests', () => {
  ...
  describe('executeCommand', async () => {
    ...
    test('when command fails, should reject', async () => {
      await expect(
        executeCommand({ command: `node -e "throw new Error('fail')"` })
      ).rejects.toThrow()
    })
  })
  ...
  describe('validateInputs', () => {
    ...
    test.each([
      ...
      'should return valid when command: %s',
      async ({ command, projects }) => {
    ...
    test.each([
      {
        ...
        projects: [],
        targets: [],
        error: 'Projects cannot be empty'
      },
      {
        command: 'runManyListedTargetsAndListedProjects',
        projects: ['projA'],
      ...
      },
      {
        ...
        projects: ['projA'],
        targets: [],
        error: 'Projects must be empty'
      }
    ])(
      'Should throw an error when inputs are %s',
      ({ command, projects, targets, error }) => {
        inputs.command = command as never
        inputs.projects = projects
